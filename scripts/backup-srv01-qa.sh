...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    -h|--help) echo "Uso: bash scripts/backup-srv01-qa.sh [--env RUTA] [--output RUTA]"; exit 0 ;;
...
if [ -z "$OUTPUT" ]; then
  OUTPUT="/opt/centenario/backups/qa-srv01-$(date -u +%Y%m%dT%H%M%SZ)"
fi
...
"${compose[@]}" run --rm -T --no-deps   -v "$OUTPUT/objects:/backup"   --entrypoint /bin/sh minio-init   -c 'set -eu; mc alias set pmgm http://minio:9000 "$MINIO_ACCESS_KEY" "$MINIO_SECRET_KEY" >/dev/null; mkdir -p /backup/pmgm-documents; mc mirror --overwrite pmgm/pmgm-documents /backup/pmgm-documents'

source_sha="unknown"
if [ -f "$ROOT/BUILD-INFO.txt" ]; then
  source_sha="$(awk -F= '$1=="SOURCE_SHA"{print $2}' "$ROOT/BUILD-INFO.txt" | tail -n1)"
elif command -v git >/dev/null 2>&1; then
  source_sha="$(git -C "$ROOT" rev-parse HEAD 2>/dev/null || printf unknown)"
fi

BACKUP_DIR="$OUTPUT" SOURCE_SHA="$source_sha" python3 - <<'PY'
...
for path in sorted(p for p in root.rglob("*") if p.is_file() and p.name!="manifest.json"):
    ...
    files.append({"path":path.relative_to(root).as_posix(),"sha256":h.hexdigest(),"sizeBytes":size})
manifest={
  "formatVersion":1,
  "project":"PMGM",
  "stack":"qa-srv01",
...
bash "$ROOT/scripts/verify-srv01-backup.sh" "$OUTPUT"
restart_runtime
resume=()
trap - EXIT
echo "RESPALDO QA SRV01 CREADO: $OUTPUT"

...
try {
    ...
    function Wait-Endpoint([string]$Url, [string]$Name, [int]$Attempts = 90) {
        ...
        throw "$Name no quedó disponible. Revise: docker compose -f infrastructure/docker-compose.first.yml logs"
    }
    ...
    Write-Host 'Para reiniciar desde cero: .\scripts\start-first-implementation.ps1 -Reset'

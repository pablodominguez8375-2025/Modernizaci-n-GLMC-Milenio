...
EXPECTED_IDS = {f"QA-{i:03d}" for i in range(1, 23)}
...
def main():
    ...
    if not isinstance(checks,list) or len(checks) != 22:
        fail("deben existir exactamente 22 controles")
    seen=set()
    failed=[]
    pending=[]
    for item in checks:
    ...
    if a.allow_pending:
        print(f"QA SRV01 TEMPLATE OK: 22 controles; pending={len(pending)}, failed={len(failed)}")
        return 0
    ...
    print("QA SRV01 REGRESSION PASS: 22/22 controles")
    return 0

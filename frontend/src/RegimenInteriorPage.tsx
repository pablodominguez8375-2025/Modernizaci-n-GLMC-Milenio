...
function AssemblyRosterDemo() {
  ...
  return <section className="panel report-wide"><div className="panel-heading"><div><p className="eyebrow">Gran Asamblea · datos ficticios</p><h2>Revisión de habilitaciones de Gran Asamblea</h2><p>Clasificación automática y alertas revisadas por Régimen Interior antes del cierre inmutable.</p></div><div><span className="count-badge">{unresolved} pendientes</span> <button className="primary-action" type="button" disabled={closed || unresolved > 0} onClick={() => setClosed(true)}>{closed ? 'Revisión cerrada · versión 1' : 'Cerrar revisión'}</button></div></div>
    ...
    <p className="report-footnote">La revisión separa habilitación para asistir y sufragar; el padrón de la Gran Asamblea corresponde a electores habilitados y nunca contiene votos individuales.</p>
  </section>
}

...
it('reads effective session capabilities from the API instead of decoding the JWT', async () => {
  const response = { displayName: 'Hermana Institucional', accessScope: 'order', capabilities: { canApproveTransfers: false, canRunRegimenInteriorReports: true, canManageGrandSecretariat: true, canManageTreasuryRegularity: false, canManageHospitalariaRegularity: false, canEvaluateCeremonies: true, canManagePrivacy: false } }
  const fetch = vi.fn().mockResolvedValue(new Response(JSON.stringify(response))); vi.stubGlobal('fetch', fetch)
  expect((await new PmgmApiClient({ getAccessToken: async () => 'token' }).getSessionProfile()).capabilities.canRunRegimenInteriorReports).toBe(true)
...
it('posts Gran Secretaria reservations with an optional ceremony link', async () => {
  ...
  const [url, options] = fetch.mock.calls[0]
  expect(url).toBe('/api/gran-secretaria/reservas'); expect(options.method).toBe('POST'); expect(options.headers.get('Content-Type')).toBe('application/json')
  expect(JSON.parse(options.body as string)).toMatchObject({ organizationId: 'o1', ceremonyRequestId: 'c1', spaceId: 's1' })
})
...
it('uses the minimized ceremony queue and formal authorization endpoints', async () => {
  ...
  const response = await client.getSecretariatCeremonyQueue()
  expect(response.items[0].formalAuthorizationIssued).toBe(false)
  expect(fetch.mock.calls[0][0]).toBe('/api/institutional/gran-secretaria/ceremonias-autorizadas')
  await client.issueSecretariatCeremonyAuthorization('c1', 'r1')
  const [url, options] = fetch.mock.calls[1]
  expect(url).toBe('/api/gran-secretaria/ceremonias/c1/autorizacion'); expect(options.method).toBe('POST'); expect(JSON.parse(options.body as string)).toEqual({ spaceReservationId: 'r1' })
})

it('never sends an institutional request without a token', async () => {
  const fetch = vi.fn(); vi.stubGlobal('fetch', fetch)
  await expect(new PmgmApiClient().getCandidatePortal()).rejects.toThrow('Debe ingresar'); expect(fetch).not.toHaveBeenCalled()
})
...
it('demo does not request token or network', async () => {
  ...
  await client.getSystemInfo(); await client.getRegimenInteriorSummary(); await client.getSecretariatAvailability('2026-09-08T18:00:00Z', '2026-09-08T20:00:00Z'); await client.getSecretariatCeremonyQueue()

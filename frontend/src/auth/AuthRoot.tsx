...
function createRuntime() {
  try {
    ...
    return { session, api, lodgeApi, documentApi, calendarApi, notificationApi, error: '' }
  } catch {
    return { session: null, api: null, lodgeApi: null, documentApi: null, calendarApi: null, notificationApi: null, error: 'El acceso institucional no está configurado. Contacte a la administración.' }
  }
}
const runtime = createRuntime()

export default function AuthRoot() {
  if (!runtime.api || !runtime.lodgeApi || !runtime.documentApi || !runtime.calendarApi || !runtime.notificationApi) return <AccessScreen message={runtime.error} />
  if (!runtime.session) return <App api={runtime.api} lodgeApi={runtime.lodgeApi} documentApi={runtime.documentApi} calendarApi={runtime.calendarApi} notificationApi={runtime.notificationApi} />
...
function AuthenticatedApp({ session }: { session: OidcSession }) {
  ...
  if (state.status === 'authenticated' && runtime.api && runtime.lodgeApi && runtime.documentApi && runtime.calendarApi && runtime.notificationApi) {
    return <App api={runtime.api} lodgeApi={runtime.lodgeApi} documentApi={runtime.documentApi} calendarApi={runtime.calendarApi} notificationApi={runtime.notificationApi} onLogout={() => { void session.logout() }} />
  }

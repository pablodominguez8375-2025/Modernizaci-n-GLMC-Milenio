...
function createRuntime() {
  try {
    ...
    return { session: null, api: null, error: 'El acceso institucional no está configurado. Contacte a la administración.' }
  }
}
const runtime = createRuntime()

export default function AuthRoot() {
  if (!runtime.api) return <AccessScreen message={runtime.error} />
  if (!runtime.session) return <App api={runtime.api} />
...
function AuthenticatedApp({ session }: { session: OidcSession }) {
  ...
  if (state.status === 'authenticated' && runtime.api) {
    return <App api={runtime.api} onLogout={() => { void session.logout() }} />
  }

...
La implementación quedó propuesta en el PR #169 hacia `dev`, desde `dev@85fc5a5816910ae9c477a0aecc1121aca21429b8`. Añade conciliaciones históricas de sólo inserción por rango, con totales calculados en servidor, cantidad de movimientos, saldo contado, diferencia, referencia/nota, sujeto y UTC; cada nuevo conteo conserva el anterior. Se mantiene el límite de 50 registros recientes por rango y se agrega QA-041. Pruebas frontend 199/199, lint/build y gates locales de matriz QA, clasificación y migraciones pasan. Backend y prueba PostgreSQL requieren CI exact-head. No se declara QA/UAT aceptada ni se interviene `srv01` o `main`.

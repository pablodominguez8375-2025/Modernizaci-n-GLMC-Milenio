...
### Corte integrado PR #168 — continuidad de auditoría

PR #168 se integró por squash en `dev` como `85fc5a5816910ae9c477a0aecc1121aca21429b8`; actualiza BASE, NEXT y QA-V067 con la integración del PR #167 y evidencia verificable de su artefacto. Sus checks exact-head terminaron SUCCESS; Pages post-merge / `qa-current.json` permanecen por confirmar. `srv01` continúa pausado y Issue #97 sigue abierto.

### Corte integrado PR #169 — conciliación de caja/banco

Tras revisar el “Sistema Logial de Ejemplo - solo como referencia”, PR #169 incorporó al reporte del Taller el guardado inmutable de cada arqueo, su diferencia reproducible, hora UTC, responsable y referencia documental opcional. El cálculo se hace desde los libros bajo snapshot transaccional PostgreSQL RepeatableRead y separa egresos pendientes. El semáforo/conciliación mensual de aportes no se duplica porque ya pertenecen al Cuadro institucional de Gran Tesorería. Se descartó la proyección de caja de la referencia porque supone cobranza íntegra y no modela de forma fiable apertura y obligaciones. No cambian permisos, aprobaciones, cuotas ni monedas.

PR #169 se integró por squash en `dev` como `2db812f37c2080d166b74692f0118f566879138c`, con base `85fc5a5816910ae9c477a0aecc1121aca21429b8`. La conciliación guarda importes calculados por el servidor, cantidad de movimientos, saldo contado, diferencia, referencia/nota, sujeto y UTC; los nuevos conteos conservan las evidencias anteriores. El historial por rango muestra hasta 50 registros y QA-041 documenta las pruebas físicas pendientes. Post-merge: PMGM CI #1559 (run 36205450888), Showcase/Pages #829 (run 36205450889), QA Installable #467 (run 36205450901) y Pre-UAT #361 (run 36205450882) SUCCESS sobre el SHA integrado. BUILD-INFO/SOURCE_SHA coincide; el artefacto QA Actions tiene digest `sha256:673d107bdb5de4cafcc265437177f22074d4c490ffe9fe5148a15dff8dc1c1cb` y MANIFEST válido. `qa-current.json` de Pages registra el mismo SHA y el ZIP servido con SHA-256 `0a8989a5aaac3bf8fc2a5ecb13dc2ca16a9b9127b30de8eccfc1774323a942be`; su BUILD-INFO y MANIFEST también validan. Los ZIP Pages y Actions se generaron por separado y tienen hashes distintos, pero ambos corresponden al mismo SOURCE_SHA. `main` sigue intacta; no se declara QA/UAT aceptada ni se interviene `srv01`.

## 24. Identidad visual institucional — actualización 25-09-2026

El Product Owner autorizó aplicar a la interfaz la *Guía de uso del logotipo de la GLMCh* y la plantilla de papelería distribuidas por Publicaciones el 18-09-2026. Para estructura/composición siguen vigentes los mockups; para el uso institucional de logo, colores y tipografías prevalecen la guía y la plantilla oficiales.

- Azul oficial del mandil/banda y color principal: `#06148E`.
- Azul complementario: `#004AD4`; azul claro: `#B4D0F4`.
- Dorados de identidad: `#F3C609` y `#FBAE17`, usados como acento/selección con texto e iconos azul oscuro para mantener contraste.
- Cambria es la familia editorial institucional de la plantilla (12 pt en documentos); la interfaz adopta Cambria con alternativas locales compatibles. Arial Narrow y rojo `#C42E00` corresponden al nombre dentro del logotipo oficial; no se debe recomponer ese logotipo con texto ni alterar su archivo, proporción, colores o espacio de protección.

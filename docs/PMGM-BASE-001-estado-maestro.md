...
### Alcance autorizado siguiente — conciliación de caja/banco

Tras revisar el “Sistema Logial de Ejemplo - solo como referencia”, se aplicarán al reporte del Taller el guardado inmutable de cada arqueo, la diferencia reproducible, la marca temporal UTC, responsable y referencia documental opcional. El cálculo lo hará el servidor desde los libros, separando egresos pendientes. El semáforo y la conciliación mensual de aportes no se duplican porque ya pertenecen al Cuadro institucional de Gran Tesorería. No se replica la proyección de caja de la referencia porque supone la cobranza íntegra de cuotas y no modela confiablemente apertura y obligaciones. No cambian permisos, aprobaciones, cuotas ni monedas.

La implementación quedó propuesta en el PR #169 hacia `dev`, desde `dev@85fc5a5816910ae9c477a0aecc1121aca21429b8`. Añade conciliaciones históricas de sólo inserción por rango, con totales calculados en servidor bajo snapshot transaccional PostgreSQL RepeatableRead, cantidad de movimientos, saldo contado, diferencia, referencia/nota, sujeto y UTC; cada nuevo conteo conserva el anterior. Se mantiene el límite de 50 registros recientes por rango y se agrega QA-041. Pruebas frontend 199/199, lint/build y gates locales de matriz QA, clasificación y migraciones pasan. Backend y prueba PostgreSQL requieren CI exact-head. No se declara QA/UAT aceptada ni se interviene `srv01` o `main`.

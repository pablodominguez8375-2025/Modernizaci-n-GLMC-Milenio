...
## Mejora autorizada — evidencia histórica de cuadratura de caja
...
El PR #169 hacia `dev`, en la rama `feature/treasury-reconciliation-audit-20260926`, agrega un registro inmutable por cuadratura y rango: el servidor recalcula saldo de apertura, ingresos, egresos aprobados/pendientes y saldo de cierre bajo una transacción RepeatableRead, compara saldo contado, cuenta movimientos y registra responsable/UTC con referencia/nota opcional. El historial admite nuevos conteos sin sobrescribir evidencia. Incluye QA-041 y clasificación de datos. No cambia permisos, política de aprobación, monedas ni libro de movimientos. Pruebas frontend 199/199, lint/build y gates locales de QA (41 pendientes), clasificación, migraciones y diff-check pasan; el entorno local no tiene .NET SDK. CI/backend, Showcase/Pages e instalable exact-head siguen pendientes antes de integrar; luego registrar la evidencia en Issue #97 y en la Línea Base Maestra de Drive. `srv01` físico, smoke autenticado allí, regresión institucional y UAT siguen pendientes por instrucción del Sponsor; no aceptar gates automáticos como sustituto.

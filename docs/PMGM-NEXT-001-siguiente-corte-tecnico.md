...
Implementación en rama: API detallada con RUT/nombres, snapshot de cargos activos al corte, columnas alineadas y datos ficticios para Pages. QA-036 registra aceptación física futura. Local: frontend 188/188, lint/build SUCCESS; JSON y gate QA de 36 controles SUCCESS. Commit local `6f456eb0118529c06c54b9f3a1b2704c9d490e4a`. El push fue bloqueado por revisión automática por riesgo de publicar código y documentación internos a GitHub sin autorización; no se abrió PR ni hubo cambios remotos. No hay .NET SDK local; backend/PostgreSQL requiere CI exact-head. Publicación de rama, PR a `dev`, CI/Showcase/instalable/Pre-UAT del mismo SHA y actualización del handoff pendientes de autorización explícita. QA física/UAT continúan pendientes por Issue #97. `main` intacta.

...
QA-036 amplía la regresión física a 36 controles. Verificaciones locales: frontend 188/188, lint/build SUCCESS, JSON de clasificación válido, gate QA 36 controles SUCCESS. Commit local `6f456eb0118529c06c54b9f3a1b2704c9d490e4a`. El push fue bloqueado por revisión automática por riesgo de publicar código y documentación internos a GitHub sin autorización; no se abrió PR ni hubo cambios remotos. .NET SDK no está instalado localmente; CI backend/PostgreSQL exact-head pendiente. Publicación, PR a `dev`, CI/Showcase/instalable/Pre-UAT y actualización de Demo/artefactos pendientes de autorización explícita. QA física `srv01` y UAT pendientes. `main` no se modifica.
